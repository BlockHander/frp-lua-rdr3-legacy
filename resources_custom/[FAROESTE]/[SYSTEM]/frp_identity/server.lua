...
AddEventHandler(
    ...
    function(cid)
        ...
        print(_source, User, cid)
        User:setCharacter(cid)
    end
)

RegisterServerEvent('FRP:IDENTITY:loadCharacterSelection')
AddEventHandler(
    'FRP:IDENTITY:loadCharacterSelection',
    function(source)
        local _source = source
        local User = API.getUserFromSource(_source)
        print(_source, User, "asd", source)
        TriggerEvent('FRP:IDENTITY:DisplayCharSeletion', User)
    end
)
