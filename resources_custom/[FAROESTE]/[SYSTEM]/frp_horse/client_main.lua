...
function WhistleHorse(whistleTypeHash)
    ...
    if cAPI.IsPlayerHorseActive() and not cAPI.IsPlayerHorseActivationBlocked() then
        ...
        if not cAPI.IsPlayerHorseActivationBlocked() and cAPI.IsPlayerInitialized() then
            ...
            cAPI.notify("error", "A lovad épp nem hallja a hívásodat, próbald meg később.")
        end
    end
end
...
function CreatePrompts(prompt_group)
    ...
    PromptSetText(prompt_inventory, CreateVarString(10, "LITERAL_STRING", "Tárhely"))
    ...
    PromptSetText(prompt_eat, CreateVarString(10, "LITERAL_STRING", "Etetés"))
    ...
    PromptSetText(prompt_drink, CreateVarString(10, "LITERAL_STRING", "Itatás"))
    ...
    PromptSetText(prompt_brush, CreateVarString(10, "LITERAL_STRING", "Fésülés"))
    ...
    function()
        while true do
            ...
            if PromptIsJustPressed(prompt_brush) then
                local User = cAPI.getUserFromSource()
                local Character = User.getCharacter()
                local BrushCount = Character.hasItem('horse_brush')
                ...
                cAPI.notify("error", "Nincs nálad ló kefe!")
            end
            end
